import streamlit as st
import gspread
from oauth2client.service_account import ServiceAccountCredentials
import random

# --- 1. KAPCSOLAT FÜGGVÉNY ---
def test_upload():
    try:
        scope = ["https://spreadsheets.google.com/feeds", "https://www.googleapis.com/auth/drive"]
        
        # A megadott JSON adatok
        creds_dict = {
            "type": "service_account",
            "project_id": "genial-acronym-489918-a6",
            "private_key_id": "85c43711722d4586bfaca301bf5e186b41d923f1",
            "private_key": """-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n""",
            "client_email": "erdo-mentes@genial-acronym-489918-a6.iam.gserviceaccount.com",
            "client_id": "111987957484558081367",
            "auth_uri": "https://accounts.google.com/o/oauth2/auth",
            "token_uri": "https://oauth2.googleapis.com/token",
            "auth_provider_x509_cert_url": "https://www.googleapis.com/oauth2/v1/certs",
            "client_x509_cert_url": "https://www.googleapis.com/robot/v1/metadata/x509/erdo-mentes%40genial-acronym-489918-a6.iam.gserviceaccount.com"
        }

        # Kapcsolódás
        creds = ServiceAccountCredentials.from_json_keyfile_dict(creds_dict, scope)
        client = gspread.authorize(creds)
        
        # Táblázat megnyitása
        url = "https://docs.google.com/spreadsheets/d/10DyyNfaYh0C9orj-F_Go8Har46Tsm1JNISzecQ1dcIc/edit#gid=0"
        sheet = client.open_by_url(url).get_worksheet(0)
        
        # Random számok generálása
        a = random.randint(1, 100)
        b = random.randint(1, 100)
        c = random.randint(1, 100)
        
        # Sor hozzáadása (fontos: listaként adjuk át)
        sheet.append_row([a, b, c])
        return f"Siker! Beküldve: {a}, {b}, {c}"

    except Exception as e:
        return f"Hiba történt: {str(e)}"

# --- 2. STREAMLIT FELÜLET ---
st.title("🧪 Google Sheets Teszt")

if st.button("TESZT SOR FELKÜLDÉSE"):
    with st.spinner("Küldés folyamatban..."):
        eredmeny = test_upload()
        if "Siker" in eredmeny:
            st.success(eredmeny)
        else:
            st.error(eredmeny)
