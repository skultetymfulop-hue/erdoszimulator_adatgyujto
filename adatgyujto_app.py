import streamlit as st
import gspread
from oauth2client.service_account import ServiceAccountCredentials

def simple_test():
    try:
        scope = ["https://spreadsheets.google.com/feeds", "https://www.googleapis.com/auth/drive"]
        
        # JSON adatok (közvetlenül beégetve a legtisztább teszthez)
        creds_dict = {
            "type": "service_account",
            "project_id": "genial-acronym-489918-a6",
            "private_key": """-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n""",
            "client_email": "erdo-mentes@genial-acronym-489918-a6.iam.gserviceaccount.com",
            "token_uri": "https://oauth2.googleapis.com/token",
        }
        
        # Kulcs javítása és hitelesítés
        creds_dict["private_key"] = creds_dict["private_key"].replace("\\n", "\n")
        creds = ServiceAccountCredentials.from_json_keyfile_dict(creds_dict, scope)
        client = gspread.authorize(creds)
        
        # Táblázat megnyitása ID alapján (ez a legbiztosabb)
        sheet = client.open_by_key("10DyyNfaYh0C9orj-F_Go8Har46Tsm1JNISzecQ1dcIc").get_worksheet(0)
        
        # Számok 1-től 10-ig egy listában
        szamok = [list(range(1, 11))] # Egy sor, 10 oszlop
        
        # Az A1:J1 tartományba írunk (ez felülírja az első sort!)
        sheet.update('A1:J1', szamok)
        
        return "Zseniális! Sikerült beírni 1-től 10-ig az első sorba!"

    except Exception as e:
        return f"Hiba: {str(e)}"

st.title("Szuper-egyszerű teszt")
if st.button("Írj 1-10-ig az első sorba"):
    eredmeny = simple_test()
    st.write(eredmeny)
